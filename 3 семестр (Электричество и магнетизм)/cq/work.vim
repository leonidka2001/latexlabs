...
silent only
silent tabonly
cd ~/Desktop/latexlabs/3\ семестр\ (Электричество\ и\ магнетизм)/cq
...
tabnew
tabrewind
edit ../../2\ семестр\ (Термодинамика\ и\ молекулярная\ физика)/choice_question/cq.tex
...
wincmd w
argglobal
if bufexists("cq.tex") | buffer cq.tex | else | edit cq.tex | endif
let s:cpo_save=&cpo
set cpo&vim
inoremap <buffer> <silent> <BS> =AutoPairsDelete()
imap <buffer> <nowait> <silent> <F7> <Plug>(vimtex-cmd-create)
...
setlocal wrap
setlocal wrapmargin=0
let s:l = 195 - ((41 * winheight(0) + 22) / 44)
if s:l < 1 | let s:l = 1 | endif
exe s:l
normal! zt
195
...
set stal=1
badd +0 cq.tex
badd +0 preamble/preamble1.tex
badd +0 ../../2\ семестр\ (Термодинамика\ и\ молекулярная\ физика)/choice_question/cq.tex
